...
# v_m ACROSS; the strap prints front-face-down and is flipped to install,
# which mirrors the pattern ONCE. Baking the compensating mirror here keeps
# bracket.scad dumb (see the chiral flip-to-use gotcha) — but the y-seam and
# x-seam cases differ: for the y-straps (u,v)=(x,y) so v_m = -v supplies the
# mirror; for the x-straps (u,v)=(y,x) is an axis SWAP, itself a reflection,
# so v_m = +v. Negating both was the vertical-strap chirality bug (S3/S4
# printed mirrored, found at assembly 2026-07-21).
def to_local(name, q):
    axis, coord = {"S1": (1, SY), "S2": (1, SY), "S3": (0, SXT), "S4": (0, SXB)}[name]
    u = q[0] if axis == 1 else q[1]
    v = (q[1] - coord) if axis == 1 else (q[0] - coord)
    return (round(u, 2), round(-v if axis == 1 else v, 2))
...
with open("src/parts/bracket_layout.scad", "w") as f:
    ...
    f.write("// LOCAL PRINTED coords: u along seam, v baked for the front-face-\n")
    f.write("// down print + flip-to-install: v = -(board v) on the y-straps,\n")
    f.write("// v = +(board v) on the x-straps (the u/v axis swap already\n")
    f.write("// mirrors there — see to_local in the generator).\n")
